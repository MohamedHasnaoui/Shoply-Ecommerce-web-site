...
import { AdminHomeStatistics, PeriodFilter } from "../../generated";
import { adminService } from "../../services/admin";

const HomeGeneralStatistics = () => {
    const [generalStatistics,setGeneralStatistics] = useState<AdminHomeStatistics | null>(null);
    ...
    useEffect(() => {
        const fetchGeneralStatistics = async () => {
            const response = await adminService.getAdminStatistics(selectedPeriod?.periodFilter);
            setGeneralStatistics(response.data.getAdminHomeStatistics);
          ...
          <div style={{cursor:"pointer"}} className="block block-rounded block-transparent bg-gd-elegance" >
            <div className="block-content block-content-full block-sticky-options">
              ...
              <div className="py-3 text-center">
                <div className="fs-2 fw-bold mb-0 text-white">{generalStatistics?.registeredBuyers?? "..."}</div>
        ...
        <div className="col-md-6 col-xl-3">
          <div style={{cursor:"pointer"}} className="block block-rounded block-transparent bg-gd-dusk">
            <div className="block-content block-content-full block-sticky-options">
              ...
              <div className="py-3 text-center">
                <div className="fs-2 fw-bold mb-0 text-white">{generalStatistics?.registeredSeller?? "..."}</div>
                <div className="fs-sm fw-semibold text-uppercase text-white-75">New Buyers</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="col-md-6 col-xl-3">
          <div style={{cursor:"pointer"}} className="block block-rounded block-transparent bg-gd-sea">
            <div className="block-content block-content-full block-sticky-options">
              ...
              <div className="py-3 text-center">
                <div className="fs-2 fw-bold mb-0 text-white">{generalStatistics?.newProducts?? "..."}</div>
                <div className="fs-sm fw-semibold text-uppercase text-white-75">New Products</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="col-md-6 col-xl-3">
          <div style={{cursor:"pointer"}} className="block block-rounded block-transparent bg-gd-aqua">
            <div className="block-content block-content-full block-sticky-options">
              ...
              <div className="py-3 text-center">
                <div className="fs-2 fw-bold mb-0 text-white">{generalStatistics?.newOrders?? "..."}</div>
                <div className="fs-sm fw-semibold text-uppercase text-white-75">New Orders</div>
              </div>
            </div>
          </div>
        </div>
