...
const RegisteredBuyersChart = (props:propsType) => {
    const [registeredBuyers, setRegisteredBuyers] = useState<Array<number> >([]);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const navigate = useNavigate();
    const fetchregisteredBuyers = async () => {
      try{
        setLoading(true);
        const response = await adminService.getRegisteredUsersByPeriod(Role.Buyer,props.period)
    ...
    useEffect(()=>{
      fetchregisteredBuyers();
    });
    const Labels: Record<PeriodFilter, Array<string | number>> = {
      [PeriodFilter.Week] : ['MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT', 'SUN'],
      [PeriodFilter.Year] : Array.from({length: registeredBuyers.length}, (_, i) => i + 1),
      [PeriodFilter.Month] : Array.from({length: registeredBuyers.length}, (_, i) => i + 1),
        ...
        <div className="block-content block-content-full bg-body-light text-center">
          <div className="row g-sm">
            <div className="col-12">
              ...
              <div className="fs-3 fw-semibold text-success">{registeredBuyers.reduce((acc, val) => acc + val, 0)}</div>
            </div>
          </div>
        </div>
...
export default RegisteredBuyersChart
